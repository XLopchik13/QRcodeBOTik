...
async def send_qr_code(request: QRCodeRequest):
    try:
        ...
        return JSONResponse(
            content={"message": "QR-код успешно отправлен"},
            status_code=200
        )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Ошибка при отправке QR-кода: {str(e)}"
        )
...
async def send_qr_code(request: QRCodeScaner):
    try:
        ...
        return JSONResponse(
            content={"message": "QR-код успешно просканирован, а данные отправлены в Telegram"},
            status_code=200
        )
